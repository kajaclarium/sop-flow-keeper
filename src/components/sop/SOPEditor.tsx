...
import { SOPFormat } from "@/types/sop";
...
export function SOPEditor({ mode }: { mode: "create" | "edit" }) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (mode === "create") {
      const id = createSop(title, format, owner || "Unassigned");
  ...
  }, [title, format, owner, mode, existingSop, fileName, aiAnalysis, createSop, updateSop, navigateToList, navigateToView, toast]);
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-4xl mx-auto px-6 py-6 space-y-6">
            ...
            <div className="inline-flex rounded-lg border bg-muted p-1">
              <button
                ...
                className={cn(
                  ...
                  format === "block" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-all",
                  format === "file" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                )}
              >
        ...
        {/* Block Builder */}
        {(mode === "create" ? format === "block" : sop?.format === "block") && (
          <div className="space-y-3">
            {(sop?.steps || [{ id: crypto.randomUUID(), instruction: "", requirePhoto: false, requireMeasurement: false }]).map(
              (step, i) => (
                <StepCard
                  key={step.id}
                  step={step}
                  index={i}
                  onUpdate={(updates) => sop && updateStep(sop.id, step.id, updates)}
                  onRemove={() => sop && removeStep(sop.id, step.id)}
                  canRemove={(sop?.steps.length || 1) > 1}
                  disabled={!canEdit}
                />
              )
            )}
            {canEdit && sop && (
              <Button variant="outline" className="w-full gap-2 border-dashed" onClick={() => addStep(sop.id)}>
        ...
        {/* File Wrapper */}
        {(mode === "create" ? format === "file" : sop?.format === "file") && (
          <FileUploadZone
